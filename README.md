...
**면접 포인트:** "클라이언트가 매 요청마다 인증서버를 거치지 않고, JWT를 통해 API Gateway에서 직접 검증합니다."
...
- **면접 포인트:** "토큰 기반 인증으로 서비스간 의존성 제거"

### API Gateway

- **역할:** 단일 진입점, 횡단 관심사 처리
- **기능:** 인증, Rate Limiting, 라우팅, Trace ID 주입
- **면접 포인트:** "모든 요청에 Trace ID를 주입하여 분산 추적 가능"

### Eureka

- **역할:** 서비스 디스커버리
- **패턴:** 클라이언트 사이드 디스커버리
- **면접 포인트:** "서비스가 자동으로 등록/해제되어 수평 확장 용이"

### Feign Client

- **역할:** 선언적 HTTP 클라이언트
- **장점:** 인터페이스 기반, Eureka 연동
- **면접 포인트:** "Spring Cloud LoadBalancer와 연동하여 클라이언트 로드밸런싱"

### Resilience4j

- **역할:** 장애 회복 라이브러리
- **패턴:** Circuit Breaker, Retry, Time Limiter, Bulkhead
- **면접 포인트:** "외부 시스템 장애가 내부로 전파되지 않도록 격리"
